# Model Garden Skills for Gemini CLI · Claude Code

Vertex AI Model Garden 오픈 모델을 AI CLI에서 쓸 수 있게 정리한 Skills 모음입니다.
Gemini CLI와 Claude Code 모두 지원하며, MaaS (서버리스 API)와 Self-Deployed (Vertex AI Endpoint·GKE) 두 가지 방식을 모두 다룹니다.
...
[Gemini CLI](https://github.com/google-gemini/gemini-cli) 또는 [Claude Code](https://claude.ai/code)와 Git이 필요합니다.
...
ln -s "$(pwd)/skills/maas-open-models" ~/.claude/skills/maas-open-models
ln -s "$(pwd)/skills/self-deploy-open-models" ~/.claude/skills/self-deploy-open-models
```
...
# Claude Code
cp -r skills/maas-open-models ~/.claude/skills/
cp -r skills/self-deploy-open-models ~/.claude/skills/
```
...
Claude Code에서는 `/maas-open-models` 또는 `/self-deploy-open-models`로 명시적으로 호출할 수 있습니다.
